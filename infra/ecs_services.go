...
// createECSServicesFromInput is a wrapper function that accepts ECSServicesInput
// and creates ECS services regardless of whether using managed or low-cost infrastructure
func createECSServicesFromInput(ctx *pulumi.Context, projectName, environment string, cluster *ECSClusterResources,
	network *NetworkingResources, securityGroups *SecurityGroups, alb *ALBResources, roles *IAMRoles,
	logs *LogGroups, repositories *ECRRepositories, secrets *SecretsResources, input *ECSServicesInput, tags pulumi.StringMap) (*ECSServicesResources, error) {
	...
	// SuperTokens Task Definition
	supertokensTaskDef, err := createSuperTokensTaskDefinitionFromInput(ctx, projectName, environment, roles, logs, input, secrets, tags)
	if err != nil {
		return nil, err
	}
	...
	// API Task Definition
	apiTaskDef, err := createAPITaskDefinitionFromInput(ctx, projectName, environment, roles, logs, repositories, input, secrets, namespace, tags)
	if err != nil {
		return nil, err
	}
	...
	// Worker Task Definition
	workerTaskDef, err := createWorkerTaskDefinitionFromInput(ctx, projectName, environment, roles, logs, repositories, input, secrets, tags)
	if err != nil {
		return nil, err
	}
...
func createSuperTokensTaskDefinitionFromInput(ctx *pulumi.Context, projectName, environment string, roles *IAMRoles,
	logs *LogGroups, input *ECSServicesInput, secrets *SecretsResources, tags pulumi.StringMap) (*ecs.TaskDefinition, error) {

	containerDef := pulumi.All(input.DatabaseEndpoint, secrets.DatabaseSecretArn, secrets.SuperTokensSecretArn, logs.SuperTokensLogGroup.Name).ApplyT(
		func(args []interface{}) (string, error) {
			endpoint := args[0].(string)
			dbSecretArn := args[1].(string)
			superTokensSecretArn := args[2].(string)
			...
			containers := []map[string]interface{}{
				{
					...
					"environment": []map[string]interface{}{
						{
							"name":  "POSTGRESQL_CONNECTION_URI",
							"value": fmt.Sprintf("postgresql://%s@%s:5432/%s", input.MasterUsername, endpoint, input.SuperTokensDBName),
						},
					},
					"secrets": []map[string]interface{}{
						{
							"name":      "POSTGRESQL_PASSWORD",
							"valueFrom": fmt.Sprintf("%s:password::", dbSecretArn),
						},
						{
							"name":      "API_KEYS",
							"valueFrom": fmt.Sprintf("%s:api_key::", superTokensSecretArn),
						},
					},
					"logConfiguration": map[string]interface{}{
						"logDriver": "awslogs",
...
func createAPITaskDefinitionFromInput(ctx *pulumi.Context, projectName, environment string, roles *IAMRoles,
	logs *LogGroups, repositories *ECRRepositories, input *ECSServicesInput,
	secrets *SecretsResources, namespace *servicediscovery.PrivateDnsNamespace, tags pulumi.StringMap) (*ecs.TaskDefinition, error) {

	containerDef := pulumi.All(repositories.APIRepoURL, input.DatabaseEndpoint, input.RedisEndpoint,
		secrets.DatabaseSecretArn, secrets.SuperTokensSecretArn, logs.APILogGroup.Name, namespace.Name).ApplyT(
		func(args []interface{}) (string, error) {
			imageURL := args[0].(string)
			dbEndpoint := args[1].(string)
			redisEndpoint := args[2].(string)
			dbSecretArn := args[3].(string)
			superTokensSecretArn := args[4].(string)
			logGroup := args[5].(string)
			namespaceName := args[6].(string)
...
func createWorkerTaskDefinitionFromInput(ctx *pulumi.Context, projectName, environment string, roles *IAMRoles,
	logs *LogGroups, repositories *ECRRepositories, input *ECSServicesInput,
	secrets *SecretsResources, tags pulumi.StringMap) (*ecs.TaskDefinition, error) {

	containerDef := pulumi.All(repositories.WorkerRepoURL, input.DatabaseEndpoint, input.RedisEndpoint,
		secrets.DatabaseSecretArn, logs.WorkerLogGroup.Name).ApplyT(
		func(args []interface{}) (string, error) {
			imageURL := args[0].(string)
			dbEndpoint := args[1].(string)
